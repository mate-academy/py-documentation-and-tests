...
class AuthenticatedMovieApiTests(TestCase):
    ...
    def test_filter_movies_by_genres_and_actors(self):
        ...
        res1 = self.client.get(MOVIE_URL, {"genres": f"{genre.id}"})
        res2 = self.client.get(MOVIE_URL, {"actors": f"{actor.id}"})
        res3 = self.client.get(MOVIE_URL, {"title": f"{test_movie.title}"})

        serializer1 = MovieListSerializer(test_movie)
        serializer2 = MovieListSerializer(empty_movie)
        ...
        self.assertIn(serializer2.data, res3.data)

    def test_retrieve_movie_detail(self):
        test_movie = sample_movie()
        test_movie.genres.add(sample_genre())
        test_movie.actors.add(sample_actor())
    ...
    def test_update_movie_forbidden(self):
        ...
        response = self.client.put(detail_url(test_movie.id), payload)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
...
class AdminMovieApiTests(TestCase):
    ...
    def test_delete_movie_not_allowed(self):
        ...
        response = self.client.delete(detail_url(test_movie.id))

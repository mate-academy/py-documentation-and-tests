...
class AuthenticatedMovieAPITest(TestCase):
    ...
    def test_list_movies(self):
        sample_movie()
        movie_with_genres = sample_movie(title="movie_with_genres")
    ...
    def test_filter_movies_by_title(self):
        movie1 = sample_movie(title="First")
        movie2 = sample_movie(title="Second")
    ...
    def test_filter_movies_by_genres(self):
        ...
        response = self.client.get(MOVIE_URL, {"genres": f"{genre2.id}"})
    ...
    def test_filter_movies_by_actors(self):
        ...
        response = self.client.get(MOVIE_URL, {"actors": f"{actor2.id}"})
    ...
    def test_retrieve_movie_detail(self):
        ...
        movie.actors.add(Actor.objects.create(first_name="Actor"))
...
class AdminMovieAPITests(TestCase):
    ...
    def test_create_movie_with_genres_actors(self):
        ...
        response = self.client.post(MOVIE_URL, payload)
        movie = Movie.objects.get(id=response.data["id"])
        genre = movie.genres.all().first()
        actor = movie.actors.all().first()

        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertIn(genre.id, response.data["genres"])
        self.assertIn(actor.id, response.data["actors"])

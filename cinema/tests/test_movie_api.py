...
class UnAuthenticatedMoviesApiTest(TestCase):
    ...
    def test_auth_required(self):
        ...
        movie = sample_movie()
        response = self.client.post(detail_url(movie.id))
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

class AuthenticatedMoviesApiTests(TestCase):
    def setUp(self):
        self.client = APIClient()
    ...
    def test_filter_movies_by_actors(self):
        ...
        actor_2 = sample_actor(first_name="Jey", last_name="Smith")
    ...
    def test_retrieve_movie(self):
        movie = sample_movie()
        movie.genres.add(sample_genre(), Genre.objects.create(name="Crime"))
        movie.actors.add(sample_actor(), Actor.objects.create(first_name="Jey", last_name="Smith"))
...
class AdminMovieApiTest(TestCase):
    ...
    def test_update_movie_not_allowed(self):
        movie = sample_movie()
        url = detail_url(movie.id)
        response = self.client.put(url, {"title": "Update Test movie"})
        self.assertEqual(response.status_code, status.HTTP_405_METHOD_NOT_ALLOWED)

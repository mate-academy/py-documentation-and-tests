...
class AuthorizedUserMovieTest(TestCase):
    ...
    def test_movie_filter_actors(self):
        test_actor_1 = Actor.objects.create(first_name="Jack", last_name="London")
        test_actor_2 = Actor.objects.create(first_name="Angel", last_name="White")
        movie_with_actors = sample_movie(title="Neo")
        movie_with_actors.actors.add(test_actor_1, test_actor_2)
        movie_without_actors = sample_movie(title="Shrek")
        serializer_no_actors = MovieListSerializer(movie_without_actors)
        serializer_actors = MovieListSerializer(movie_with_actors)

        response = self.client.get(MOVIE_LIST_URL, {f"actors": f"{test_actor_2.id},{test_actor_1.id}"})
    ...
    def test_movie_filter_genres(self):
        test_genre_1 = Genre.objects.create(name="Triller")
        ...
        movie_without_genre = sample_movie(title="Shrek")
...
class AdminUserMovieTest(TestCase):
    ...
    def test_post_with_actors_success(self):
        ...
        response = self.client.post(MOVIE_LIST_URL, movie_info)
        movie = Movie.objects.get(id=response.data["id"])
        movie.actors.add(test_actor_1, test_actor_2)
        self.assertEqual(movie.actors.count(), 2)
        self.assertIn(test_actor_1, movie.actors.all())
